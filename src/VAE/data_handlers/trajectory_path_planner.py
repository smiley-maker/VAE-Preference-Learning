...
class PathPlanner(Dataset):
    ...
    def plan_path(sem_map, costs, start, end, device):
        ...
        print("Remapping...")
        value_to_label = dict(zip(np.unique(sem_map), costs[:len(np.unique(sem_map))]))
        cost_map = np.vectorize(value_to_label.get)(sem_map) 
        print("created costmap")

        # Creates a mapping of terrain types to indices
        mapping = {
        ...
        terrains = torch.tensor([mapping[sem_map[x[0]][x[1]]] for x in path], dtype=torch.float, device=device)
